...
if ($LASTEXITCODE -ne 0) {
    throw 'MVSep Mega53 model assets are not verified. Run Install-MVSep-Mega53-v335.cmd.'
}
& $Python -m tools.verify_bs_roformer_mega53_runner --model-dir $env:JUWEIER_BS_ROFORMER_MODEL_DIR --quick
if ($LASTEXITCODE -ne 0) {
    throw 'BS-RoFormer runner is outdated. Run Install-BS-RoFormer-Mega53-v336.cmd.'
}

Write-Host 'Juweier Music v3.3.6 server pilot starting on port 8001'
Write-Host 'Base six stems: direct Demucs htdemucs_6s with validated PCM WAV'
Write-Host 'Dedicated guitars: MVSep Mega53 with compatible mlp_expansion_factor=2 runner'
