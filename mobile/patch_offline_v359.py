...
if "package:path_provider/path_provider.dart" not in text:
    text = text.replace("import 'package:just_audio/just_audio.dart';", "import 'package:just_audio/just_audio.dart';\nimport 'package:path_provider/path_provider.dart';", 1)

# Persistent local product state.
anchor = "  bool stemMixerActive = false;"
if anchor not in text:
    raise SystemExit('stemMixerActive field missing')
text = text.replace(anchor, anchor + "\n  final Set<int> offlineReady = <int>{};\n  final Map<int, double> offlineDownloadProgress = <int, double>{};", 1)

# Use application support storage instead of systemTemp, so downloaded products survive app/page restarts.
text = re.sub(
...
# Add reusable offline-download helpers before setStemEnabled.
    ...
    try {
      for (final entry in assets.entries) {
        ...
        await _downloadToFile(url, target, token, (part) {
          offlineDownloadProgress[job.libraryId] = ((done + part) / assets.length).clamp(0, 1);
          if (mounted) setState(() {});
        });
        done++;
      }
...
# Local filenames generated by loadPlayers must match explicit offline package names.
text = text.replace("url, 'original_${job.libraryId}', 'mp3',", "url, 'original', 'mp3',", 1)

# Clear, labelled Play/Pause transport button.
...
# Add explicit offline product action below current-song selector.
...
# Mini player play/pause also gets an explicit label.
text = text.replace(
...
# On successful full stem takeover, mark product ready if all files already cached locally.
text = text.replace("      stemMixerActive = true;\n      _advanceLoading(1, '分轨混音已接管播放');", "      stemMixerActive = true;\n      if (job.libraryId > 0 && await _hasOfflineProduct(job)) offlineReady.add(job.libraryId);\n      _advanceLoading(1, '分轨混音已接管播放');", 1)
