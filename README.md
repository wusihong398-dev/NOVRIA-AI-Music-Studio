...
- 新增独立电吉他轨道入口；基础模型仍生成六轨，检测到二阶段电吉他结果时自动升级为完整七轨。
...
| 端 | 目录 / 入口 | 说明 |
|---|---|---|
| Windows | `app/`、`Run-Juweier-Music.bat` | 本地服务器/测试工作站，负责七轨兼容、分析、编配、谱面和演出 |
...
第一次分轨会下载并校验 `htdemucs_6s` 模型。Windows EXE 可通过 GitHub Actions 的 **Build Juweier Music Windows EXE** 生成；产物名为 `Juweier-Music-v3.1.0-Windows-x64`，其中含安装程序和便携 ZIP。
