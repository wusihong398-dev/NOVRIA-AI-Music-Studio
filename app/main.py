...
from app.library_taxonomy import DEFAULT_CATEGORIES, DISCOVERY_TABS
...
class SeparationWorker(QThread):
    ...
    def _ensure_model(self):
        ...
        self.log.emit("首次使用：正在下载 UVR htdemucs_6s 六轨模型...")
    ...
    def run(self):
        try:
            ...
            self.log.emit(f"UVR 运算设备：{device.upper()}")
            ...
            self.separation_progress.emit(100, "UVR 六轨 + 独立电吉他分离完成")
            self.log.emit(
                "UVR 六轨模型处理完成；已生成独立木吉他与电吉他轨。"
...
class ServerCatalogWorker(QThread):
    ...
    def __init__(self, request_id, client, query, category, initial="全部", since=0):
        super().__init__()
    ...
    def run(self):
        try:
            self.succeeded.emit(
                self.request_id,
                self.client.songs(
                    self.query, self.category, initial=self.initial, since=self.since,
                ),
            )
        except Exception as exc:
...
class ServerArtifactCacheWorker(QThread):
    """Download published AI assets in the background so opening a song never freezes."""
        ...
        with self._io_lock:
            for key, f in self.files.items():
                ...
                if key in active and n and output_count:
                    ...
                    mix[:output_count] += rendered * float(self.volume[key]) * float(self.master_volume)
            if self.files:
...
class StudioPage(QWidget):
    def __init__(self, main):
        ...
        title = QLabel("橘味儿音乐库 · 多轨演奏 / 乐器工作台")
        ...
        master_row.addWidget(QLabel("🔊 总音量"))
        self.master_volume = QSlider(Qt.Horizontal)
        self.master_volume.setRange(0, 120)
        self.master_volume.setValue(90)
        self.master_volume_label = QLabel("90%")
        self.master_volume.valueChanged.connect(main.sync_master_volume)
        master_row.addWidget(self.master_volume, 1)
        ...
        self.model_status = QLabel("首次使用时会下载 UVR htdemucs_6s；完成后生成独立电吉他轨")
...
class SettingsPage(QWidget):
    def __init__(self, main):
        ...
        server_box = QGroupBox("橘味儿服务器（已发布成品来源）")
        ...
        server_layout.addWidget(QLabel("只读取服务器已发布成品索引；客户端不会扫描任何本地硬盘。"), 3, 0, 1, 4)
        layout.addWidget(server_box)

        legal_box = QGroupBox("协议、帮助与关于")
        legal_layout = QHBoxLayout(legal_box)
        documents = {
            "开源软件声明": "本软件使用 PySide6、FastAPI、PyTorch、Demucs、audio-separator/UVR、NumPy、SciPy、librosa 等开源组件。各组件的版权与许可条款以随包许可证及官方声明为准。",
            "隐私政策": "软件默认读取橘味儿音乐库已发布成果；仅在用户主动导入测试音频时处理或上传音频。",
            "用户协议": "本软件目前仅供学习与研究使用，不提供歌曲下载服务。只能导入自己创作、已获授权或依法可使用的音频；AI 结果须人工复核。",
            "帮助与反馈": "如遇到分轨、谱面、歌词或连接问题，请记录软件版本、歌曲格式、操作步骤和错误信息后反馈。",
            "关于软件": f"{APP_NAME} v{VERSION}\n打开橘味儿音乐库即可使用七轨、五线谱、六线谱和逐音符同步歌词。",
        }
    ...
    def save_server(self):
        ...
        QMessageBox.information(self, "橘味儿音乐库", "服务地址已保存，正在同步已发布成品曲库。")
    ...
    def test_server(self):
        try:
            ...
            QMessageBox.information(
                ...
                f"成品存储：{root}\n"
                f"已发布成品：{catalog.get('count', 0)} 首\n"
                f"歌词识别模型：{lyric_asr}",
            )
        except Exception as exc:
            QMessageBox.critical(self, "橘味儿音乐库连接失败", str(exc))
...
class SetlistPage(QWidget):
    CONFIG_FILE = BASE_DIR / "config" / "performance_albums.json"

    def __init__(self, main):
        super().__init__()
        self.main = main
        self.albums = {"我的常用曲目": []}
        self.current_album = "我的常用曲目"
        self.items = self.albums[self.current_album]
        self._load_albums()
        layout = QVBoxLayout(self)
        ...
        self.album_combo.addItems(self.albums.keys())
        self.album_combo.currentTextChanged.connect(self.select_album)
        new_album_btn = QPushButton("新建专辑")
        new_album_btn.clicked.connect(self.create_album)
        add_current_btn = QPushButton("加入当前歌曲")
        ...
        for w in [self.album_combo, new_album_btn, add_current_btn, add_btn, remove_btn, up_btn, down_btn, load_btn, self.auto_next]:
            bar.addWidget(w)
        bar.addStretch(1)
        ...
        tips = QLabel("把经常演奏的橘味儿音乐加入专辑，可调整顺序并随时载入多轨演奏。")
    ...
    def _load_albums(self):
        try:
            ...
            if isinstance(albums, dict) and albums:
                self.albums = {
                    str(name): [dict(item) for item in items if isinstance(item, dict)]
                    for name, items in albums.items() if isinstance(items, list)
                }
                if not self.albums:
                    self.albums = {"我的常用曲目": []}
                self.current_album = str(data.get("current") or next(iter(self.albums)))
                if self.current_album not in self.albums:
                    self.current_album = next(iter(self.albums))
                self.items = self.albums[self.current_album]
        except Exception:
            pass
    ...
    def _save_albums(self):
        ...
        atomic_write_json(self.CONFIG_FILE, {
            "current": self.current_album, "albums": self.albums,
        })

    def select_album(self, name):
        name = str(name or "").strip()
        if not name or name not in self.albums:
            return
        self.current_album = name
        self.items = self.albums[name]
        self._save_albums()
        self.refresh()

    def create_album(self):
        name, ok = QInputDialog.getText(self, "新建我的演奏专辑", "专辑名称")
        name = str(name or "").strip()
        if not ok or not name:
            return
        if name in self.albums:
            QMessageBox.information(self, "我的演奏专辑", "这个专辑已经存在。")
            return
        self.albums[name] = []
        self.album_combo.addItem(name)
        self.album_combo.setCurrentText(name)
        self._save_albums()
    ...
    def add_current_song(self):
        ...
        if not path or not Path(path).exists():
            QMessageBox.information(self, "我的演奏专辑", "请先从橘味儿音乐库选择一首歌曲。")
            self.main.open_g_drive_library()
            return
        if path not in [item["path"] for item in self.items]:
            item = {"path": path, "mode": "全部开启", "key": "原调", "speed": "100%"}
            if getattr(self.main, "selected_library_source", "") == "server":
                item["server_track_id"] = int(getattr(self.main, "selected_library_track_id", 0) or 0)
    ...
    def load_selected(self):
        ...
        if 0 <= r < len(self.items):
            item = self.items[r]
            track_id = int(item.get("server_track_id", 0) or 0)
            if track_id:
                self.main.load_server_library_track(track_id)
            else:
                self.main.load_imported_working_file(item["path"])
            QMessageBox.information(self, "我的演奏专辑", "歌曲已载入多轨演奏工作台。")
...
class DesktopScoreCanvas(QWidget):
    ...
    def paintEvent(self,event):
        ...
        if self.show_lyrics:
            ...
            painter.drawText(QRectF(24,self.height()-55,width-48,42),Qt.AlignCenter,lyric or "当前歌曲没有可用歌词")
        if not notes:
...
class ScorePerformancePage(QWidget):
    def __init__(self, main):
        ...
        self.lyrics_toggle = QCheckBox("显示歌词")
...
class InstrumentExperiencePage(QWidget):
    ...
    def _build_electric_guitar_tab(self):
        ...
        tips=QLabel("UVR 兼容六轨模型先生成真实 Guitar stem，再进行木吉他/电吉他二阶段识别；排练时选择电吉他手会自动关闭电吉他原轨。")
        tips.setWordWrap(True); tips.setObjectName("SectionHint")
        l.addWidget(tips,5,0,1,2)
        return w
    ...
    def save_instrument_profile(self):
        if not self.main.song_file:
            QMessageBox.warning(self,"提示","请先从成品曲库选择歌曲。")
            return
...
class UniversalImportPage(QWidget):
    ...
    def download_selected_link(self):
        ...
        try:
            ...
            dest=downloads/name
            req=urllib.request.Request(url,headers={"User-Agent":"Juweier-Music/3.5.0"})
...
class MusicLibraryPage(QWidget):
    def __init__(self, main):
        ...
        self.catalog_taxonomy = {"tabs": list(DISCOVERY_TABS), "categories": list(DEFAULT_CATEGORIES)}
        self.discovery_mode = "乐库"
        ...
        layout=QVBoxLayout(self)
        title=QLabel("橘味儿乐库 · 服务器预处理成果")
        title.setObjectName("PageTitle")
        layout.addWidget(title)

        hint=QLabel(
            "曲库由服务器统一整理、批量 AI 处理并发布；三端读取同一份增量索引，离线时继续显示本地缓存。"
            "用户打开歌曲即可使用歌词、五线谱、六线谱、和弦与独立乐器轨。"
        )
        hint.setObjectName("SectionHint")
        hint.setWordWrap(True)
        layout.addWidget(hint)
        ...
        for b in [choose_root,refresh]:
            top.addWidget(b)
        top.addStretch(1)
        layout.addLayout(top)

        discovery_row=QHBoxLayout()
        discovery_row.addWidget(QLabel("发现"))
        self.discovery_buttons={}
        for tab_name in DISCOVERY_TABS:
            button=QPushButton(tab_name)
            button.setCheckable(True)
            button.setChecked(tab_name=="乐库")
            button.clicked.connect(lambda checked=False, item=tab_name: self._select_discovery_tab(item))
            self.discovery_buttons[tab_name]=button
        ...
        self.library_category.addItems(list(DEFAULT_CATEGORIES))
        ...
        for value in ["全部", *list("ABCDEFGHIJKLMNOPQRSTUVWXYZ"), "#"]:
            ...
            button.setFixedWidth(50 if value == "全部" else 36)
        ...
        library_box=QGroupBox("橘味儿音乐成品分类库（歌手默认折叠，展开后按需显示歌曲）")
        ...
        pipeline_box=QGroupBox("服务器批量生产中心")
        ...
        self.pipe_delete_selected=QPushButton("删除所选记录")
        self.pipe_clear_finished=QPushButton("清理成功/失败")
        ...
        self.pipe_delete_selected.clicked.connect(self.delete_selected_pipeline_job)
        self.pipe_clear_finished.clicked.connect(self.clear_finished_pipeline_jobs)
        ...
        pgl.addWidget(self.pipe_delete_selected,2,3)
        pgl.addWidget(self.pipe_clear_finished,2,4)
        pgl.addWidget(self.pipeline_status,2,5)
        ...
        self.library_scan_status.setText("服务器曲库：先显示本地缓存，再后台同步增量索引…")
    ...
    def scan_imports(self):
        ...
        QApplication.processEvents()
        self.library_scan_status.setText("正在请求服务器管理员同步原版歌曲目录…")
        try:
            ...
            self.progress.setValue(0)
            suffix = "；仍显示上次缓存" if self.remote_rows else ""
            self.library_scan_status.setText(f"服务器同步失败：{exc}{suffix}")
    ...
    def _load_cached_catalog(self):
        ...
        try:
            ...
            self.catalog_taxonomy = dict(payload.get("taxonomy") or self.catalog_taxonomy)
            self._render_library_rows(rows, "已显示上次保存的服务器索引，正在后台检查更新")
            return True
        except Exception:
            return False
    ...
    def refresh_library(self):
        ...
        if not self.remote_rows and not query.strip() and category == "全部":
            self._load_cached_catalog()
        self._catalog_request_id += 1
        request_id = self._catalog_request_id
        self.library_scan_status.setText("正在后台读取服务器歌曲索引，界面可继续操作…")
        since = self.catalog_version if not query.strip() and category == "全部" and self.library_initial == "全部" else 0
        worker = ServerCatalogWorker(
            request_id, self._server_client(timeout=30), query, category,
            self.library_initial, since,
        )
    ...
    def _catalog_loaded(self, request_id, result):
        ...
        if result.get("source_scope") != "server":
            self._catalog_failed(request_id, "服务器未返回 server 曲库标识")
            return
        self.server_library_root = str(result.get("server_library_root") or self.server_library_root)
        self.catalog_version = int(result.get("catalog_version") or self.catalog_version)
        self.catalog_taxonomy = dict(result.get("taxonomy") or self.catalog_taxonomy)
        categories = [str(item) for item in result.get("categories", []) if str(item)]
        if categories:
            current = self.library_category.currentText()
            self.library_category.blockSignals(True)
            self.library_category.clear()
            self.library_category.addItems(categories)
            self.library_category.setCurrentText(current if current in categories else "全部")
            self.library_category.blockSignals(False)
        if result.get("not_modified"):
            self._render_library_rows(list(self.remote_rows.values()), "服务器索引无变化")
            return
        rows = [dict(row) for row in result.get("songs", [])]
        if result.get("incremental"):
            merged=dict(self.remote_rows)
            for track_id in result.get("deleted_ids", []):
                merged.pop(int(track_id),None)
            for row in rows:
                merged[int(row["id"])]=row
            rows=list(merged.values())
        query=self.library_search.text() if hasattr(self,"library_search") else ""
        category=self.library_category.currentText() if hasattr(self,"library_category") else "全部"
        if not query.strip() and category == "全部":
            try:
                atomic_write_json(self.catalog_cache_file, {
                    "songs": rows,
                    "server_library_root": self.server_library_root,
                    "catalog_version": self.catalog_version,
                    "taxonomy": self.catalog_taxonomy,
    ...
    def _catalog_failed(self, request_id, error):
        if int(request_id) != self._catalog_request_id:
            return
        suffix = "；当前继续显示已缓存歌曲" if self.remote_rows else ""
        self.library_scan_status.setText(
            f"橘味儿音乐库读取失败：{error}{suffix} · 不会回退扫描客户端硬盘"
        )
    ...
    def _render_library_rows(self, all_rows, status_prefix=""):
        ...
        if self.discovery_mode != "歌手":
            visible_limit=600
            for row in rows[:visible_limit]:
                state=str(row.get("processing_status") or "待处理")
                tags=" · ".join((str(row.get("language") or ""),str(row.get("genre") or ""))).strip(" ·")
                item=QTreeWidgetItem([
                    f'{row.get("artist") or "未知歌手"} - {row.get("title") or "未命名歌曲"}',
                    f'{tags} · {state}'
                ])
                item.setData(0,Qt.UserRole,("track",int(row["id"])))
                self.tree.addTopLevelItem(item)
            prefix = f"{status_prefix} · " if status_prefix else ""
            more = f" · 首屏显示前 {visible_limit} 首，可搜索全部歌曲" if len(rows)>visible_limit else ""
            self.library_scan_status.setText(
                f"{prefix}橘味儿音乐库已发布 {len(rows)} 首歌曲{more} · 本页没有扫描任何硬盘"
            )
        ...
        for artist in sorted(artists, key=lambda value: value.casefold()):
            ai=QTreeWidgetItem([artist,f"{len(artists[artist])} 首"])
            ai.setData(0,Qt.UserRole,("artist",artist))
            placeholder=QTreeWidgetItem(["展开后加载歌曲",""])
        ...
        if rows:
            prefix = f"{status_prefix} · " if status_prefix else ""
            self.library_scan_status.setText(
                f"{prefix}当前显示 {len(artists)} 位歌手、{len(rows)} 首服务器歌曲 · "
                f"服务器索引：{self.server_library_root} · 未扫描客户端硬盘"
            )
        self.tree.setUpdatesEnabled(True)
    ...
    def _select_discovery_tab(self, value):
        self.discovery_mode = str(value)
        for key, button in self.discovery_buttons.items():
            button.setChecked(key == value)
        mapping = {"推荐": "推荐", "乐库": "全部", "AI成果": "AI已完成"}
        if value in mapping:
            self.library_category.setCurrentText(mapping[value])
        elif value == "歌手":
            self.library_category.setCurrentText("全部")
            self.library_search.setFocus()
        elif value == "分类":
            self.library_category.showPopup()
        elif value == "歌单":
            self.library_category.setCurrentText("推荐")
    ...
    def delete_selected_pipeline_job(self):
        row=self.pipeline_table.currentRow()
        if row < 0 or row >= len(self.pipeline_jobs):
            QMessageBox.information(self,"删除流水线记录","请先选择一首歌曲记录。")
            return
        if row == self.pipeline_job_index and self.pipeline_running:
            QMessageBox.information(self,"删除流水线记录","当前歌曲正在处理，请暂停或等待完成后再删除。")
            return
        self.pipeline_jobs.pop(row)
        self._save_pipeline_jobs()
        self.refresh_pipeline_table()

    def clear_finished_pipeline_jobs(self):
        before=len(self.pipeline_jobs)
        self.pipeline_jobs=[
            job for job in self.pipeline_jobs
            if str(job.get("status")) not in {"完成","失败","completed","failed"}
        ]
        self._save_pipeline_jobs()
        self.refresh_pipeline_table()
        self.pipeline_status.setText(f"已清理 {before-len(self.pipeline_jobs)} 条成功/失败记录")

    def start_auto_pipeline(self):
        if self.pipeline_executor.currentText().startswith("服务器"):
            try:
                result=self._server_client(timeout=30).start_library_batch()
                counts=dict(result.get("counts") or {})
                self.pipeline_status.setText(
                    f"服务器后台流水线已启动：待处理 {counts.get('pending',0)} · "
                    f"已完成 {counts.get('ready',0)} · 关闭本软件后服务器仍会继续"
                )
                self.pipe_pause.setText("暂停流水线")
            except Exception as exc:
                QMessageBox.critical(self,"服务器流水线启动失败",str(exc))
            return
    ...
    def toggle_pipeline_pause(self):
        if self.pipeline_executor.currentText().startswith("服务器"):
            try:
                result=self._server_client(timeout=30).pause_library_batch()
                self.pipeline_status.setText(str(result.get("message") or "服务器将在当前歌曲完成后暂停"))
                self.pipe_pause.setText("继续流水线")
            except Exception as exc:
                QMessageBox.critical(self,"暂停服务器流水线失败",str(exc))
            return
    ...
    def resume_failed_pipeline_jobs(self):
        if self.pipeline_executor.currentText().startswith("服务器"):
            try:
                result=self._server_client(timeout=30).start_library_batch(retry_failed=True)
                counts=dict(result.get("counts") or {})
                self.pipeline_status.setText(
                    f"服务器失败任务已重新排队：待处理 {counts.get('pending',0)}"
                )
            except Exception as exc:
                QMessageBox.critical(self,"重试服务器任务失败",str(exc))
            return
        count=0
...
class CommunityPage(QWidget):
    ...
    def __init__(self, main):
        ...
        form.addWidget(QLabel("音乐库服务"), 0, 0)
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.nav.item(0).setHidden(True)
        self.nav.setCurrentRow(1)
        side.addWidget(self.nav, 1)

        song_context=QGroupBox("当前橘味儿音乐")
    ...
    def open_g_drive_library(self):
        ...
        self.statusBar().showMessage("请按歌手展开并双击歌曲，可直接使用服务器分轨、歌词和谱面")
    ...
    def load_server_library_track(self, track_id, row=None):
        ...
        if not row:
            try:
                payload = self.music_library._server_client(timeout=60).songs()
                row = next(
                    (dict(item) for item in payload.get("songs", [])
                     if int(item.get("id", 0) or 0) == int(track_id)),
                    {},
                )
                if row:
                    self.music_library.remote_rows[int(track_id)] = row
            except Exception:
                row = {}
        if not row:
            QMessageBox.warning(self, "橘味儿音乐库", "找不到这首歌曲，请同步最新曲库后重试。")
            return
        ...
        try:
            ...
            self.current_song_label.setText(f"橘味儿音乐 · {artist}\n{title}")
            artifact_urls=dict(row.get("artifacts") or {})
            self.selected_server_artifacts=artifact_urls
            if artifact_urls:
                self.statusBar().showMessage(f"歌曲已打开，正在后台缓存 AI 成果：{artist} - {title}")
                worker=ServerArtifactCacheWorker(
                    self.music_library._server_client(timeout=180), artifact_urls,
                    cache_dir/f"{int(track_id)}_artifacts",
                )
                if not hasattr(self,"server_artifact_workers"):
                    self.server_artifact_workers=[]
                self.server_artifact_workers.append(worker)
                worker.done.connect(lambda files, folder=cache_dir/f"{int(track_id)}_artifacts": self._server_artifacts_ready(folder,files))
                worker.failed.connect(lambda error: self.statusBar().showMessage(f"歌曲可播放，AI 成果后台缓存失败：{error}"))
                worker.finished.connect(lambda current=worker: self._release_server_artifact_worker(current))
                worker.start()
            else:
                self.statusBar().showMessage(f"当前服务器歌曲：{artist} - {title} · 等待 AI 预处理")
        except Exception as exc:
            QMessageBox.critical(self, "服务器歌曲加载失败", str(exc))

    def _server_artifacts_ready(self, folder, files):
        folder=Path(folder)
        self.selected_server_artifacts=dict(files or {})
        if all((folder/f"{name}.wav").is_file() for name in (
            "vocals","drums","bass","guitar","electric_guitar","piano","other"
        )):
            self.stem_dir=folder
            self.base_stem_dir=folder
            try:
                self.engine.load(folder)
                self.load_waveform_if_ready()
                self.sync_mix_controls()
            except Exception as exc:
                self.statusBar().showMessage(f"服务器音轨已缓存，但混音器载入失败：{exc}")
                return

        score_path=Path(str(self.selected_server_artifacts.get("score_data") or folder/"score_data.json"))
        if score_path.is_file():
            try:
                score=json.loads(score_path.read_text(encoding="utf-8"))
                self.analysis_result={
                    "bpm": float(score.get("bpm",120) or 120),
                    "key": str(score.get("key") or "C"),
                    "duration": float(score.get("duration",0) or 0),
                }
                self.chord_timeline=list(score.get("bars") or [])
                self.melody_reference=list(score.get("staff_notes") or [])
                lyric_units=list(score.get("lyric_units") or [])
                lyric_rows=list(score.get("lyrics") or [])
                self.lyric_reference=[
                    {
                        "start": float(row.get("start",row.get("time",0)) or 0),
                        "end": float(row.get("end",row.get("start",row.get("time",0))) or 0),
                        "text": str(row.get("text") or row.get("word") or ""),
                    }
                    for row in (lyric_units or lyric_rows)
                    if str(row.get("text") or row.get("word") or "").strip()
                ]
                self.studio.bpm_label.setText(f"BPM：{self.analysis_result['bpm']:.1f}")
                self.studio.key_label.setText(f"调性：{self.analysis_result['key']}")
            except Exception as exc:
                self.statusBar().showMessage(f"音轨可用，但服务器谱面数据读取失败：{exc}")
                return
        if hasattr(self,"score_performance"):
            self.score_performance.refresh_score()
        self.statusBar().showMessage("橘味儿音乐成品已就绪：七轨、逐音符歌词和乐谱可直接使用")

    def _release_server_artifact_worker(self, worker):
        if worker in getattr(self,"server_artifact_workers",[]):
            self.server_artifact_workers.remove(worker)
        worker.deleteLater()
    ...
    def sync_master_volume(self, value):
        self.engine.master_volume = max(0.0, min(1.2, float(value) / 100.0))
        self.studio.master_volume_label.setText(f"{int(value)}%")

    def play_pause(self):
        if not self.engine.files:
            QMessageBox.warning(self, "提示", "这首成品的多轨文件尚未加载完成，请返回橘味儿音乐库刷新后重试。")
            return
        try:
...
<part id="P1">{''.join(measures)}</part></score-partwise>"""
    ...
    def closeEvent(self, event: QCloseEvent):
        ...
        workers.extend(list(getattr(self,"server_artifact_workers",[])))
